...
================================================================================
 ...
 variables or read them from a file (an ifstream object must be given).
...
------------  Start sample input -------------------------
   ...
   DT		= 2.5; 		// time step
   MAX_STEPS = 100ULL;	// Maximum time to simulate
   MAX_TRAJ = 100ULL;	// Max number of trajectories to run
------------  End sample input -------------------------
 
 NOTE:
 1. Key and the corresponding value must be on the
...
inline bool is_not_blank(
   const string& s,
   const string& delimiters = " \f\n\r\t\v" )
{
   string::size_type n = s.find_first_not_of(delimiters);
   return n != string::npos && s != "";
}
//===========================================================================
...
SpinBosonInput::SpinBosonInput()
{
   ...
   OMEGA	= 1.0e0;	// Freq. of incident radiation
   DT		= 2.5; 		// time step
   MAX_STEPS = 100;	// Maximum time to simulate
   MAX_TRAJ = 100;	// Max number of trajectories to run
}
   ...
   *this = SpinBosonInput();
   ...
   // Define a string to hold the lines
    ...
    while ( getline(InpStream, line)  ) 
    {
       ...
       size_t loc_of_assignment_op = line.find_last_of("=:");
